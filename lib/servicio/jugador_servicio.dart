...
class JugadorServicio {
  ...
  Future<Map<String, dynamic>> listarOpcionBanco() async {
    try {
      final response = await Dioclient.dio.get(
        "${ApiConst.controlador_jugador}${ApiConst.listarOpcionBanco}",
      );
      return response.data;
    } on DioException catch (e) {
      // Si el servidor responde (ej. 401 Unauthorized), devolvemos su JSON real
  ...
  Future<Map<String, dynamic>> ejecutarAccionBanco(
    int opcionBancoId,
    int jugadorId,
  ) async {
    try {
      final response = await Dioclient.dio.put(
        ApiConst.controlador_jugador,
        data: {"opcionBancoId": opcionBancoId, "jugadorId": jugadorId},
      );
      return response.data;
    } on DioException catch (e) {
      // Si el servidor responde (ej. 401 Unauthorized), devolvemos su JSON real
